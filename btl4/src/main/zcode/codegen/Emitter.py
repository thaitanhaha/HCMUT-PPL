...
class Emitter():
    ...
    def getJVMType(self, inType):
        ...
        if typeIn is NumberType:
            ...
            # jvm_types = []
            # for x in inType.partype:
            #     print(x)
            #     jvm_types.append(self.getJVMType(x))
            # return "(" + "".join(jvm_types) + ")" + self.getJVMType(inType.rettype)
    ...
    *   @param toLabel the ending label  of the scope where the variable is active.
    ...
    def emitREADVAR(self, name, inType, index, frame):
        ...
        elif type(inType) is ArrayType or type(inType) is StringType or type(inType) is cgen.ClassType:
    ...
    *   @param in the type of the returned expression.
    ...
    def emitRETURN_HELP(self, in_):
        # in_: Type
        # frame: Frame

        if type(in_) is NumberType:
            return self.jvm.emitFCONST("0.0") + self.jvm.emitFRETURN()
        elif type(in_) is StringType:
            return self.jvm.emitLDC("\"0\"") + self.jvm.emitARETURN()
        elif type(in_) is BoolType:
            return self.jvm.emitICONST(1) + self.jvm.emitIRETURN()
        elif type(in_) is ArrayType:
            return self.emitARRAY_NO_FRAME(in_) + self.jvm.emitARETURN()
    ...
    *   .super java/lang/Object<p>
    ...
    def emitEPILOG(self):
        ...
        file.write(''.join(self.sub_buff))
        file.write(''.join(self.var_sub_buff))
        for i in self.buff:
            if type(i) is list:
                file.write(''.join(i))
            else:
    ...
    *   @param in the code to be printed out
    ...
    def sub_printout(self, in_):
        self.sub_buff.append(in_)

    def var_sub_printout(self, in_):
        self.var_sub_buff.append(in_)

    def clearBuff(self):
        self.buff.clear()

    def emitARRAY(self, in_, frame):
        # in_: ArrayType
        typ = in_.eleType
        sizes = in_.size
        for size in in_.size[::-1]:
            typ = cgen.ArrayType([size], typ)

        if type(in_.eleType) is not ArrayType:
            pass
        else:
            temp_size = []
            flat_typ = in_
            while type(flat_typ.eleType) is ArrayType:
                temp_size.append(flat_typ.size[0])
                flat_typ = flat_typ.eleType
            temp_size.append(flat_typ.size[0])
            sizes = temp_size[::-1]

        result = [self.emitPUSHICONST(int(size), frame) for size in sizes]
        [frame.pop() for _ in sizes]
        if len(result) == 1:
            if type(in_.eleType) is cgen.StringType:
            ...
            result.append(self.jvm.emitMULTIANEWARRAY(self.getJVMType(typ), str(len(result))))
        else:
            raise IllegalOperandException(str(in_))
    ...
    def emitARRAY_NO_FRAME(self, in_):
        result = [self.jvm.emitICONST(1)]
        typ = in_
        while type(typ.eleType) is ArrayType:
        ...
        if len(result) == 1:
            ...
            result.append(self.jvm.emitMULTIANEWARRAY(self.getJVMType(in_), str(len(result))))
        else:
            raise IllegalOperandException(str(in_))
        return ''.join(result)
